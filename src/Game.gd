...
func on_item_collected(item_type: Globals.ItemType):
	if item_type == Globals.ItemType.DIRECTION:
		...
		rpc("update_client_item_count", current_special_power_items, item_type)
			# Gegner zufällig spawnen
		#randomize()
		#if true:
		#if randi() % 2 == 0:  # 50% Wkt
			#if bat_enemy_scene:
				#var bat_instance = bat_enemy_scene.instantiate()
				
				# Spawnposition setzen
				#var spawn_position = Vector2.ZERO
				#if Globals.control_mode == Globals.ControlMode.INDIVIDUAL:
				#	if players.size() > 0 and is_instance_valid(players[0]):
				#		spawn_position = players[0].global_position
				#elif Globals.control_mode == Globals.ControlMode.SHARED:
				#	if is_instance_valid(shared_player):
				#		spawn_position = shared_player.global_position

				#bat_instance.global_position = spawn_position
				#add_child(bat_instance)
				#print("Gegner gespawnt bei ", spawn_position)
			#else:
				#print("bat_enemy_scene ist null. Szene nicht gesetzt!")
	#hide_enemies_for_lightplayer()
...
func _on_control_button_pressed() -> void:
	...
	#print("Overlay sichtbar:", ControlOverlay.visible)
